...
int count = 1;
void my_function(int start, int stop, CP::map_bst<int,int> &bst){
    int between = (start+stop)/2;
    bst[between] = 100;
    //std::cout << count << " " << "START " << start << " STOP " << stop << " Between " << between << std::endl;
    count++;
    if(stop - start == 1) return;
    my_function(start, between, bst);
    my_function(between+1, stop, bst);
}
