  ...
  public:
    ...
    void reorder(int pos,std::vector<int> selected) {
      //write your code only here
        ...
        int num = 0;
        for(auto x:selected){
            list_iterator posit = list_iterator(mHeader->next);
            list_iterator it = list_iterator(mHeader->next);
            if(x < pos) x = x - num;
            for(int i = 0; i < x; i++) it.ptr = it.ptr->next;
            for(int j = 0; j < pos+count; j++) posit.ptr = posit.ptr->next;
            it.ptr->next->prev = it.ptr->prev;
            it.ptr->prev->next = it.ptr->next;

            posit.ptr->prev->next = it.ptr;
            it.ptr->next = posit.ptr;
            
            it.ptr->prev = posit.ptr->prev;
            posit.ptr->prev = it.ptr;
            num++;
            if(x > pos) count += 1;
        }
    }
